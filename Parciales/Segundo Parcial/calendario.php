...
<body>
	...
	<table cellpadding="0" cellspacing="2" width="200" border="0" style="text-align: center;">
		...
		<tr height="17">
			<?php foreach ($spDays as $dd)
				echo '<th bgstyle="color:#4069B1" scope="col">' . $dd . '</th>'; ?>
		</tr>
		<?php
		if ($v < 7) {
			?>
			<tr><?php
			foreach ($fDays2Use as $k => $v)
				echo '<td bgstyle="color:#F0F5FF">' . $v . '</td>';
			?>
			<tr><?php
		}
		reset($days2Use);
		for ($x = 1; $x <= $top; $x++) {
			...
			for ($p = 1; $p <= 7; $p++) {
				?>
					<td bgstyle="color:#<?php echo $bgcolor ?>"><?php echo (current($days2Use)) ? current($days2Use) : "&nbsp;" ?>
					</td><?php
					next($days2Use);
			}
			?>
